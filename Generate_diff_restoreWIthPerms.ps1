...
--E: drive resides on P054SQLCI2101.uscust.local,8881\ECA01 - use this to restore to ECA01
--set @DatafileDestination = 'E:\SQL01_DATA_MP01\MSSQL\DATA\'  --untested
--set @DatafileDestination = 'E:\SQL01_DATA_MP02\MSSQL\DATA\'  --untested
--set @DatafileDestination = 'E:\SQL01_DATA_MP03\MSSQL\DATA\'
  --set @DatafileDestination = 'E:\SQL01_DATA_MP04\MSSQL\DATA\'
  --set @LogfileDestination = 'E:\SQL01_LOGS_MP01\MSSQL\Logs\'

 --F: drive resides P054SQLCI2102.USCUST.local,8882\ECA02 - USE THIS TO RESTOER TO ECA02
 --set @DatafileDestination = 'F:\SQL02_DATA_MP01\MSSQL\DATA\'
 --set @DatafileDestination = 'F:\SQL02_DATA_MP02\MSSQL\DATA\'
 --set @DatafileDestination = 'F:\SQL02_DATA_MP03\MSSQL\DATA\'
...
set @script2 = @script2 + char(10) + ' DECLARE @sql VARCHAR(max) 
	...
	IF EXISTS (select name from sys.database_principals where name = ''''AMER\OCR'''')
	 ...
	 IF EXISTS (select name from sys.database_principals where name = ''''Amer\svc_docprod'''')
	   BEGIN
			grant connect to [USCUST\svc_DocProd_preview]
			exec sp_addrolemember ''''roleimageviewer'''', ''''USCUST\svc_DocProd_Preview'''' 
		     If EXISTS (select name from sys.schemas where name = ''''amer\svc_docprod'''')
			    BEGIN
				  ALTER AUTHORIZATION ON SCHEMA::[Amer\svc_docprod] TO [USCUST\svc_DocProd_preview]
	...
	IF EXISTS (select name from sys.database_principals where name = ''''AMER\SVC_CLaimsMatrixWeb'''')
	  BEGIN
		  grant connect to [USCUST\svc_CMWeb_preview]
				IF EXISTS (select name from sys.schemas where name = ''''AMER\svc_ClaimsMatrixWeb'''')
					BEGIN
						ALTER AUTHORIZATION ON SCHEMA::[Amer\SVC_CLaimsMatrixWeb] TO [USCUST\svc_CMWeb_preview]
						DROP SCHEMA [AMER\SVC_CLaimsMatrixWeb]
					END
			exec sp_addrolemember ''''roleCAuser'''', ''''USCUST\svc_CMweb_preview''''
	...
	IF EXISTS (select name from sys.database_principals where name = ''''POORMAN-DOUGLAS\lexprocess'''')
	    BEGIN
			grant connect to [USCUST\svc_LexProcess_previ]
				IF EXISTS (select name from sys.schemas where name = ''''POORMAN-DOUGLAS\lexprocess'''')
				   BEGIN
					ALTER AUTHORIZATION ON SCHEMA::[POORMAN-DOUGLAS\lexprocess] TO [USCUST\svc_LexProcess_previ]
					DROP SCHEMA [POORMAN-DOUGLAS\lexprocess]
				   END
			exec sp_addrolemember ''''roleimageviewer'''', ''''USCUST\svc_LexProcess_previ'''' 			
...
try
 {
    $password = read-host "enter USCUST netowrk password"
